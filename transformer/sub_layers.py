...
class SelfAttention(nn.Module):
    def __init__(self, embed_dim, d_k, d_v, mask=False, device="cpu"):
        super(SelfAttention, self).__init__()
        self.query_embed = nn.Linear(embed_dim, d_k, device=device)
        self.key_embed = nn.Linear(embed_dim, d_k, device=device)
        self.value_embed = nn.Linear(embed_dim, d_v, device=device)
        self.d_k = d_k
        self.mask = mask
...
class LayerNorm(nn.Module):
    ...
    def __init__(self, features, eps=1e-6, device="cpu"):
        ...
        self.scale = nn.Parameter(torch.ones(features)).to(device)
        self.shift = nn.Parameter(torch.zeros(features)).to(device)
        self.eps = eps
...
class PositionWiseFeedForward(nn.Module):
    def __init__(self, embed_dim, output_dim, device="cpu"):
        super(PositionWiseFeedForward, self).__init__()
        self.l1 = nn.Linear(embed_dim, output_dim, device=device)
        self.RELU = nn.ReLU()
        self.l2 = nn.Linear(output_dim, embed_dim, device=device)
